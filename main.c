...
#define RTC_BASE        0x40002800
#define RTC_TR          (*(volatile uint32_t*)(RTC_BASE + 0x00))
#define RTC_DR          (*(volatile uint32_t*)(RTC_BASE + 0x04)) // <-- DATE register
#define RTC_ISR         (*(volatile uint32_t*)(RTC_BASE + 0x0C))
#define RTC_PRER        (*(volatile uint32_t*)(RTC_BASE + 0x10))
#define RTC_WPR         (*(volatile uint32_t*)(RTC_BASE + 0x24))
...
/* receives string like: "12:34:56 31/03/26" */
void uart_receive(char *buf)
{
    int i = 0;
    while(1)
...
void rtc_set_from_uart(char *buf)
{
    ...
    uint8_t d=(buf[9]-'0')*10+(buf[10]-'0');
    uint8_t mo=(buf[12]-'0')*10+(buf[13]-'0');
    uint8_t y=(buf[15]-'0')*10+(buf[16]-'0');
...
{
    uint32_t t = RTC_TR;
    uint32_t dt = RTC_DR;   // MUST read immediately after TR
...
int main()
{
    ...
    uart_receive(buf);          // take input once
    rtc_set_from_uart(buf);    // set RTC
